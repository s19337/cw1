...
namespace LinqCwiczenia
{
    public partial class Form1 : Form
    {
        ...
        */
        ...
        {
            var groupBy = Emps.GroupBy(emp => emp.Job).Select(emp => new
            {
                Praca = emp.Key,
                LiczbaPracownikow = emp.Count()
            });

            ResultsDataGridView.DataSource = groupBy.ToList();
        }
        ...
        {
            var result = (Emps
            .Where((emp, indx) => emp.Job == "Frontend programmer")
        ...
        {
            Emp el = new Emp();
            el.Ename = "Brak wartości";
            var l = new List<Emp>();
            l.Add(el);

            IEnumerable<Emp> le = l;
            
            var l1 = le.Select(emp => new
            {
                emp.Ename,
                emp.Job,
                emp.HireDate
